...
class StaffDb(object):
    ...
    def insert(self, user: Staff) -> Staff:
        ...
        if not self.find(filter_dic):
            ...
            return Staff.User.from_dict(data)

    def insert_or_update(self, user: Staff) -> Staff:
        filter_dic = {'uuid': user.uuid}
        if not self.find(filter_dic):
            user.id = self.collection.insert_one(user.get_map_object()).inserted_id
            user_data = self.find(filter_dic)
            return Staff.User.from_dict(user_data)
        else:
            self.update(filter_dic, user.get_map_object())
            user_data = self.find(filter_dic)
            test = Staff.User.from_dict(user_data)
            return test
